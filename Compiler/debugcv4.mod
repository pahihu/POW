...
(******************************************************************************)
(* PURPOSE:  Create CodeView 4 compatible debug information                   *)
(*                                                                            *)
(******************************************************************************)
...
CONST
  FORMAT_SIGNATURE=1; (* debug information format code *)
  ...
  MAXTYPEINX=07FFFH; (* largest possible debug type index value *)
...
CONST
  TYPEINXSIZE=2; (* size of TypeInxT in bytes *)
...
BEGIN
  elem:=list.head;
  WHILE elem#NIL DO
    Coff.WriteFixupWord(elem.offset,typeInx);
    elem:=elem.next;
  END;
END Resolve;
...
BEGIN
  ...
  ELSE
    OPM.CommentedErr(E.INTERNAL_MURKS,"debugInfo.typeListT.Resolve");
  END;
END Resolve;
...
BEGIN
  ...
  IF i>=MAXINDTYPES THEN
    OPM.CommentedErr(E.INTERNAL_MURKS,"debugInfo.typeListT.GetNewId");
    RETURN -1;
  ELSE
    list.ids[i]:=TRUE;
    NEW(rList);
...
BEGIN
  ...
  IF rList=NIL THEN
    OPM.CommentedErr(E.INTERNAL_MURKS_WARN,"debugInfo.typeListT.Append");
  ELSE
    rList.Append(offset);
  END;
END Append;
...
BEGIN
  len:=S.Length(txt);
  IF len>255 THEN 
    OPM.CommentedErr(E.INTERNAL_MURKS,"DebugCV4.WriteStr");
  ELSE
...
BEGIN
  ...
  Coff.WriteObjWord(11+S.Length(name));
...
BEGIN
  Coff.WriteObjWord(9+S.Length(obj^.name));
...
BEGIN
  ...
  END;
  Coff.WriteObjWord(36+S.Length(name));
  ...
  Coff.WriteObjWord(obj^.debugInfo.type);    (* procedure type *)
...
BEGIN
  IF obj^.debugInfo#NIL THEN
    type:=SHORT(obj^.debugInfo^.type);
    IF type>=1000H THEN
      Coff.WriteObjWord(5+S.Length(obj^.name));
      Coff.WriteObjWord(S_UDT);
      Coff.WriteObjWord(type);
      SWriteStr(obj^.name);
    END;
  ELSE
    OPM.CommentedErr(E.INTERNAL_MURKS_WARN,"DebugCV4.SWriteType");
  END;
END SWriteType;
...
PROCEDURE TWriteRecLen(len:LONGINT; VAR typeInx:LONGINT);
BEGIN
  typeInx:=typeInxCtr;
  INC(typeInxCtr);
...
PROCEDURE TWriteTypeInx(typeInx:LONGINT);
BEGIN
  IF typeInx=-1 THEN
    OPM.CommentedErr(E.INTERNAL_MURKS_WARN,"DebugCV4.TWriteTypeInx");
    (* type index not initialized to either an actual or a temporary value *)
  ELSIF typeInx<0 THEN
    typeRefList.Append(SHORT(typeInx),Coff.globObjLen);
    Coff.WriteObjWord(0);
  ELSE
    Coff.WriteObjWord(typeInx);
  END;
END TWriteTypeInx;
...
BEGIN
  ...
  baseDebug:=SHORT(struct^.BaseTyp^.debugType);
  ...
  ELSE
    TWriteRecLen(6,struct.debugType);
    Coff.WriteObjWord(LF_POINTER);
    Coff.WriteObjWord(000AH); (* ptr type: near 32 bit pointer, ptr mode: pointer *)
    TWriteTypeInx(baseDebug);
    TWriteRecEnd;
  END;
END TWritePointer;

PROCEDURE TWriteFieldList(struct:OPT.Struct; VAR typeInx:LONGINT);
VAR
  hobj:OPT.Object;
  size:INTEGER;
  ...
  BEGIN
    ...
    WHILE hobj#NIL DO
      IF hobj^.mode=Fld THEN
        ...
        pad:=(-len) MOD ALIGNMENT;
        size:=size+len+pad+10+TYPEINXSIZE;
      END;
      hobj:=hobj.link;
    END;
  END CalcRecLength;
...
BEGIN
  ...
  TWriteRecLen(10+TYPEINXSIZE+6+1+S.Length(name),struct.debugType);
  ...
  Coff.WriteObjWord(0); (* CV: should be set to 0 by compiler *)
  Coff.WriteObjWord(0); (* type index of virtual function table shape descriptor *)
...
BEGIN
  ...
  WriteSubRecord(12,SHORT(struct^.BaseTyp^.debugType),ARRAYFIELD);
  TWriteRecEnd;
  TWriteRecLen(10+TYPEINXSIZE+6+1+S.Length(TYPENAME),struct.debugType);
  Coff.WriteObjWord(LF_CLASS);
...
BEGIN
  ...
  Coff.WriteObjWord(LF_FIELDLIST);
  WriteSubRecord(8,0075H,LENFIELD);
  WriteSubRecord(0,SHORT(ptrStruct^.debugType),ARRAYFIELD);
  TWriteRecEnd;
  TWriteRecLen(10+TYPEINXSIZE+6+1+S.Length(TYPENAME),object^.debugInfo^.type);
  Coff.WriteObjWord(LF_CLASS);
  Coff.WriteObjWord(2); (* CV: number of elements including methods and data members *)
  TWriteTypeInx(fieldListInx);
  Coff.WriteObjWord(0); (* property list including: forward defs? nested classes? *)
...
BEGIN
  ...
  IF staticObjMode THEN
    ...
    ptrField:=PTRFIELD1;
    typeName:=TYPENAME1;
    ptrFieldType:=SHORT(ptrStruct^.debugType);
    size:=8;
  ELSE
    ptrField:=PTRFIELD2;
    typeName:=TYPENAME2;
    ptrFieldType:=SHORT(object^.typ^.debugType);
...
BEGIN
  ...
  END;
  TWriteRecLen(11+TYPEINXSIZE,struct^.debugType);
  Coff.WriteObjWord(LF_ARRAY);
  TWriteTypeInx(SHORT(struct^.BaseTyp^.debugType));
  Coff.WriteObjWord(ARRAY_INX_TYPE);
  Coff.WriteObjWord(08004H); (* unsigned long numeric leaf: *)
...
BEGIN
  ...
  END;
  typeRefList.Resolve(tmpId,SHORT(struct.debugType));
END TWriteTypeS;

PROCEDURE TWriteType(obj:OPT.Object);
BEGIN
  IF obj^.debugInfo=NIL THEN
...
BEGIN
  ...
  WHILE hobj#NIL DO 
    ...
    hobj:=hobj^.link;
  END;
  TWriteRecLen(4+nParams*TYPEINXSIZE,typeInx);
  Coff.WriteObjWord(LF_ARGLIST);
  Coff.WriteObjWord(nParams);
  hobj:=obj; 
  WHILE hobj#NIL DO 
...
BEGIN
  ...
  ELSE
    callConvention:=2;
  END;
  TWriteRecLen(8+TYPEINXSIZE,struct^.debugType);
  Coff.WriteObjWord(LF_PROCEDURE);
  IF struct^.BaseTyp^.debugType=0 THEN
    Coff.WriteObjWord(T_VOID);
  ELSE
    Coff.WriteObjWord(struct^.BaseTyp^.debugType);
  END;
...
BEGIN
  ...
  ELSE
    callConvention:=2;
  END;
  TWriteRecLen(6+2*TYPEINXSIZE,obj^.debugInfo^.type);
  Coff.WriteObjWord(LF_PROCEDURE);
