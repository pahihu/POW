...
(*                                                                            *)
(* UPDATE                                                                     *)
(*   2002APR24 KlS Proc Number debugged     (error 03.001)                    *)
(*                                                                            *)
(******************************************************************************)
...
VAR                                    (* name, str, numtyp, intval, realval, lrlval are implicit results of Get *)
  name*:                               Name;
  str*:                                String;
...
BEGIN 
  ...
  LOOP OPM.Get(ch);
    ...
    IF ch < " " THEN Err(Error.ILLEGAL_CHAR_STRING); EXIT END ;
    IF i = MaxStrLen-1 THEN Err(Error.STRING_TOO_LONG2); EXIT END ;
    str[i]:=ch; INC(i)
  END ;
...
BEGIN 
  ...
  REPEAT
    ...
    INC(i); 
    OPM.Get(ch)
  UNTIL (((ch<"0") OR ("9"<ch)) & ((CAP(ch)<"A") OR ("Z"<CAP(ch))) & (ch#"_" (*!*))) 
    OR (i=MaxIdLen);
  IF i=MaxIdLen THEN 
    Err(Error.IDENT_TOO_LONG); 
    DEC(i) 
  END ;
  name[i]  := 0X; 
  sym      := ident
...
(******************************************************************************)
PROCEDURE Number;                    (* reads a number from source file *)
...
BEGIN (* ("0"<=ch) & (ch<="9") *)
  i  :=  0; 
  m  :=  0; 
  n  :=  0;                          (* counter for ARRAY dig[24] *)
  d  :=  0;                          (* position of decimal point; 0 = INTEGER *)
  ...
  LOOP (* read mantissa *)
    IF ("0"<=ch) & (ch<="9") OR (d=0) & ("A"<=ch) & (ch<="F") THEN
      IF (m>0) OR (ch#"0") THEN (* ignore leading zeros *)
        IF n<LEN(dig) THEN 
          ...
          INC(n);
        ELSE
          Err(Error.NUMBER_TOO_LARGE)
        END (*  IF n<LEN(dig) *);
        INC(m)
      END (* IF (m>0) OR (ch#"0"); ignore leading zeros *);
      OPM.Get(ch); 
      INC(i);
      IF d>0 THEN                    (* 2002APR24, KlS *)
        INC(d);
      END;                           (* 2002APR24, KlS *)
    ELSIF ch = "." THEN 
      OPM.Get(ch);                   (* REAL detected *)
      IF ch="." THEN                 (* ellipsis *)
        ch   := 7FX;
        EXIT
      ELSIF d=0 THEN                 (* i>0 *)
    ...
    ELSE 
      EXIT
    END (* IF ("0"<=ch) & (ch<="9") OR (d=0) & ("A"<=ch) & (ch<="F") *);
  END (* LOOP *); (* 0 <= n <= m <= i, 0 <= d <= i *)
  
  IF d=0 THEN                        (* no deciml point found: integer *)
    IF n=m THEN 
      intval :=  0; 
      i      :=  0;
      IF ch="X" THEN                 (* character *) 
        ...
        IF n <= 2 THEN
          WHILE i < n DO 
            ...
            INC(i);
          END;
        ELSE Err(Error.NUMBER_TOO_LARGE)
        END
  ...
  ELSE                               (* there is a decimal point: fraction *)
    f        :=  0; 
    e        :=  0; 
    expCh    := "E";
    numtyp   := real;
(*    WHILE n>0 DO (* 0 <= f < 1 *)     2002APR24, KlS
      DEC(n); 
      f:=(Ord(dig[n], FALSE) + f)/10;
    END  WHILE n>0 DO                   2002APR24, KlS *);
    IF n<LEN(dig) THEN               (* 2002APR24, KlS *)
      dig[n]   := 0X;
    ELSE
      dig[n-1] := 0X
    END;
    FOR i:=0 TO n-1 DO
      f  := f*10 + Ord(dig[i], FALSE);
    END (* FOR i:=0 TO n-1 *);
    WHILE d>1 DO
      f := f/10;
      DEC(d);
    ...
    IF (ch="E") OR (ch="D") THEN 
      ...
      IF ("0"<=ch) & (ch<="9") THEN
        ...
        UNTIL (ch < "0") OR ("9" < ch);
        IF neg THEN e:=-e END
      ELSE Err(Error.ILLEGAL_CHAR_NUM)
      END
    END (* IF (ch="E") OR (ch="D") *);
    DEC(e, i-m);                     (* 2002APR24, KlS; decimal point shift *)
    IF expCh="E" THEN 
      numtyp := real;
  ...
  BEGIN 
    ...
    LOOP
      ...
      IF ch=")" THEN 
        ...
        EXIT 
      END ;
      IF ch = OPM.Eot THEN Err(Error.COMMENT_OPEN); EXIT END
    END (* LOOP *)
  END Comment;
