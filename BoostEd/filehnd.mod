...
(*                     automatic version incrementation "V 2.00.21" is       *)
...
VAR
  ...
  Done:                                WinDef.BOOL;
  dwRead:                              WinDef.DWORD;       (* Anzahl der gelesenen Bytes *)
...
BEGIN   
  ...
  WHILE NextChar(char) DO
    IF pos>=ListSt.MAXLENGTH THEN   
      ...
      pos          := ListSt.MAXLENGTH-5;                  (* 2004OCT17, KlS *)
      buffer[pos]  := ">";
      INC(pos);
      buffer[pos]  := ">";
      INC(pos);
  ...
  WHILE MySyntaxColouring#NIL DO
    IF Syntax.PosInFileName(InfoString, MySyntaxColouring.Extension, 1)>0 THEN
      Options.ActSyntaxColouring   := MySyntaxColouring;
    END (* IF Strings.Pos(Extension, name, 1)>0  *);
  ...
  WHILE i>0 DO;
    IF win.Name[i]="." THEN
      Strings.Copy(win.Name, win.Extension, i+1, Strings.Length(win.Name)-i);
      i  := 0;
    END (* IF FileName[i]="."  *);
    DEC(i);
  END (* WHILE i>0 *);
  Done         := WinUser.InvalidateRect(hEdit,NIL,WinDef.True);  
  Done         := WinUser.UpdateWindow(hEdit);             (* forciert WM_PAINT-Nachricht *)
...
(*****************************************************************************)
PROCEDURE SaveFile*(hEdit:WinDef.HWND; name: WinDef.LPSTR):INTEGER;
...
VAR 
  ...
  dwWritten:                           WinDef.DWORD;       (* Anzahl geschriebener Bytes *)
                                                           (* 2001OCT03, KlS *)
