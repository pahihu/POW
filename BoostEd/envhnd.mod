...
BEGIN
  ...
  IF Strings.Pos(SECTION2, AllSections, 1)=0 THEN
    RETURN
  END (* IF Strings.Pos(SECTION2, AllSections, 1)=0 *);
  ...
  END (* WHILE ActSyntaxColouring#NIL *);
  ...
  REPEAT
    ...
    END (* IF WinBase.GetPrivateProfileStringA(... *);
  UNTIL Done;
END ReadIniFile;
...
BEGIN
  ...
  Options.ActSyntaxColouring := Options.AnchorSyntaxColouring;
  i                          :=  0;
  WHILE Options.ActSyntaxColouring^.Next#NIL DO
    Strings.Str(i, MySection);
    IF i<10 THEN
      Strings.InsertChar("0", MySection, 1);
    END (* IF i<10  *);
    ...
    END (* FOR j:=1 TO Options.ActSyntaxColouring.NoOfKeyWords *);
    ResultBool   := WinBase.WritePrivateProfileStringA(SYSTEM.ADR(MySection),
                                                       SYSTEM.ADR("KeyWords"),
                                                       SYSTEM.ADR(MyString),
                                                       SYSTEM.ADR(IniFileName));
    INC(i);
    Options.ActSyntaxColouring := Options.ActSyntaxColouring^.Next;
  END (* WHILE Options.ActSyntaxColouring^.Next#NIL  *);
END WriteIniFile;
