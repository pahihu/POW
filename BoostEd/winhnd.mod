    ...
    ELSIF message=WinUser.WM_SETFOCUS THEN
      ...
      ELSE
        WHILE MySyntaxColouring#NIL DO
          IF Syntax.PosInFileName(win.Extension, MySyntaxColouring.Extension, 1)>0 THEN
      ...
      END (* IF Strings.Length(win.Extension)=0  *);
      CreateCaret(win);
