...
BEGIN
  WHILE ~fAbort & (WinUser.PeekMessageA(msg, 0, 0, 0, WinUser.PM_REMOVE)#0) DO
    IF WinUser.IsDialogMessageA(hwndPDlg, msg)=0 THEN
      ...
      ok       := WinUser.DispatchMessageA(msg);
      (* Nachricht senden *)
    END (* IF WinUser.IsDialogMessageA(hwn *);
  END (* WHILE ~fAbort & (WinUser.PeekMe *);
  IF fAbort THEN
    RETURN 0
  ELSE
   RETURN 1
...
BEGIN
  ...
  ELSE
    ...
    res        := WinGDI.LineTo(hDCPrinter, xMax, currentY + (lineHeight*3) DIV 2);
  END (* IF Options.printLineNumbers *);
...
BEGIN
  ...
  END (* IF (printMode=MODE_ALL) OR (pri *);
END ShouldPrintPage;
...
BEGIN
  IF Options.printLineNumbers & ShouldPrintPage(pageNr) THEN
    ...
    printedOnCurrentPage := TRUE;
    prevFont   := WinGDI.SelectObject(hDCPrinter, prevFont);
  END (* IF Options.printLineNumbers & S *);
  ...
  IF textOffs + lineWidth - 1>xMax THEN                    (* Text länger als maximale Breite *)
    ...
    WHILE (textInx<len) & ShouldPrintPage(pageNr) DO
      ...
      WHILE textOffs + lineWidth - 1>xMax DO
        ...
        ok     := WinGDI.GetTextExtentPoint32A(hDCPrinter, SYSTEM.ADR(text)+textInx, lenSubStr, size);
        lineWidth := size.cx;
      END (* WHILE textOffs + lineWidth - 1> *);
      ...
      IF EndOfPage(yPos) THEN
        IF ~StartNewPage(pageNr, yPos, lineHeight) THEN
          GlobWin.Beep;
        END (* IF ~StartNewPage(pageNr, yPos,  *);
      END (* IF EndOfPage(yPos) *);
    END (* WHILE (textInx<len) & ShouldPri *);
  ...
  END (* IF textOffs + lineWidth - 1>xMa *);
  (*  Process.Yield;*)
END PrintLine;
...
(*---------------------------------------------------------------------------*)
PROCEDURE GetStringFromAdr            (adr:                LONGINT;
...
BEGIN
  ...
  ELSE
    printMode  := MODE_ALL;
  END (* IF SYSTEM.BITAND(printDlg.Flags *);
  ...
  END (* IF xOffs<WinGDI.GetDeviceCaps(h *);
  ...
  END (* IF yOffs<WinGDI.GetDeviceCaps(h *);
  ...
  IF xMax>xSizePage THEN
    xMax       := xSizePage
  END (* IF xMax>xSizePage *);
  IF yMax>ySizePage THEN
    yMax       := ySizePage
  END (* IF yMax>ySizePage *);
  ...
  END (* IF (hFont=0) OR (hItalicFont=0) *);
  ...
  END (* IF printMode=MODE_SELECTION *);
  ...
  WHILE (copyNr<=copies) & (~fAbort) & (pError#WinGDI.SP_ERROR) DO
    ...
    ELSE
      ...
      ELSE
        yExtSoFar := yOffs;
        IF ShouldPrintPage(actPage) THEN
          PrintHeader(actPage, yExtSoFar)
        END (* IF ShouldPrintPage(actPage) *);
      END (* IF WinGDI.StartPage(hDCPrinter) *);
    END (* IF printJobId<=0 *);
    ...
    WHILE (actLine<=lastLine) & (~fAbort) & (pError#WinGDI.SP_ERROR) DO
      ...
      IF EndOfPage(yExtSoFar) THEN
        ...
        IF ~StartNewPage(actPage, yExtSoFar, lineHeight) THEN
          GlobWin.Beep;
        END (* IF ~StartNewPage(actPage, yExtS *);
      END (* IF EndOfPage(yExtSoFar) *);
      IF win.text.GetLine(actLine, lineTxt, lineLen) THEN
        PrintLine(actLine, actPage, lineTxt, yExtSoFar);
      END (* IF win.text.GetLine(actLine, li *);
      INC(actLine);
    END (* WHILE (actLine<=lastLine) & (~f *);
    ...
    IF (~fAbort) THEN
      ...
      ELSE
        ...
        END (* IF WinGDI.EndDoc(hDCPrinter)<=0 *);
      END (* IF printedOnCurrentPage & (WinG *);
    ELSE
      (* Druck abbrechen *)
      ...
      END (* IF pError=WinGDI.SP_ERROR *);
    END (* IF (~fAbort) *);

    INC(copyNr);
  END (* WHILE (copyNr<=copies) & (~fAbo *);
