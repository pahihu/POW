  ...
  CASE message OF
    ...
    WinUser.WM_COMMAND:   
      ...
      IF Code=ID_OKFNT THEN  
        IF WinUser.IsDlgButtonChecked(hDlg, ID_DISP_FIXEDSYS) = 1 THEN (* ist markiert *)
          COPY(Options.FONT_FIXEDSYS,Options.fontName);
        ELSIF WinUser.IsDlgButtonChecked(hDlg, ID_DISP_COURIER) = 1 THEN (* ist markiert *)
          COPY(Options.FONT_COURIER,Options.fontName);
        ELSIF WinUser.IsDlgButtonChecked(hDlg, ID_DISP_COURIERNEW) = 1 THEN (* ist markiert *)
          COPY(Options.FONT_COURIERNEW,Options.fontName);
        END;
        IF WinUser.IsDlgButtonChecked(hDlg, ID_PRNT_FIXEDSYS) = 1 THEN (* ist markiert *)
          COPY(Options.FONT_FIXEDSYS,Options.printerFontName);
        ELSIF WinUser.IsDlgButtonChecked(hDlg, ID_PRNT_COURIER) = 1 THEN (* ist markiert *)
          COPY(Options.FONT_COURIER,Options.printerFontName);
        ELSIF WinUser.IsDlgButtonChecked(hDlg, ID_PRNT_COURIERNEW) = 1 THEN (* ist markiert *)
...
PROCEDURE [_APICALL] EditOptionsDlgProc*(hDlg:WinDef.HWND; 
                                         message: WinDef.UINT; 
                                         wParam:WinDef.WPARAM; 
                                         lParam:WinDef.LPARAM): WinDef.BOOL;
...
VAR
  ...
  hChild,hChildfirst : WinDef.HWND;
  Done               : WinDef.BOOL;
  code               : WinDef.WORD;
