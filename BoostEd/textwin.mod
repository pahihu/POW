...
TYPE
  ...
  WinDescT*            = RECORD
    ...
    SyntaxColouring*:                  Options.SyntaxColouringP;
    text*:                             ListSt.Text;        (* Text                                       *)
    hwnd*:                             WinDef.HWND;        (* Fensterhandle                              *)
    changed*:                          BOOLEAN;            (* Änderungen am Text ?                       *)
    row*,
    col*:                              LONGINT;            (* aktuelle Cursorposition                    *)
    delrow*,
    delcol*:                           LONGINT;            (* Cursorpos. des zuletzt gelöschten Elements *)
    ...
    wndheight*,
    wndwidth*:                         LONGINT;            (* Geometrie des Fensters                     *)
    ...
    undoCol*:                          LONGINT;
    undoToRow*,
    undoToCol*:                        LONGINT;            (* Undo - Bereich                             *)
    undoAction*:                       LONGINT;
    undoLen*:                          LONGINT;
    ...
    markDown*:                         BOOLEAN;
    mouseX*,
    mouseY*:                           LONGINT;            (* Mausposition während Markierung            *)
...
BEGIN
  ...
  (* Caret vom Bildschirm entfernen *)
...
(*****************************************************************************)
PROCEDURE (VAR win: WinDescT) RowVisible*
                                      (VAR row:            LONGINT)
                                      :BOOLEAN;
...
(*****************************************************************************)
PROCEDURE (VAR win: WinDescT) ColVisible*
                                      (VAR col:            LONGINT)
                                      :BOOLEAN;
...
BEGIN
  done         := WinUser.GetClientRect(win.hwnd, rect);
  (* Größe des Clientbereichs ermitteln *)
  win.wndwidth := rect.right;
  win.wndheight := rect.bottom;
  res          := WinGDI.GetTextMetricsA(win.hdc, tm);
  (* Daten über aktuelle Schrift auslesen *)
...
BEGIN
  done         := WinUser.ShowScrollBar(win.hwnd, WinUser.SB_VERT, WinDef.True);
      ...
      WHILE i<Length DO
        IF Syntax.IsIdentChar(MyText[i]) THEN
          ...
          WHILE ((Options.ActSyntaxColouring.KeyWords[j, 0]<MyText^[i]) & (j<=Options.ActSyntaxColouring.NoOfKeyWords)) DO
            INC(j);
          END (* WHILE ((Options.ActSyntaxColour *) ;
          WHILE ((Options.ActSyntaxColouring.KeyWords[j, 0]=MyText^[i]) & (j<=Options.ActSyntaxColouring.NoOfKeyWords)) DO
            Pos2 := i;
            ...
            ELSE
              i := Pos2;
            END (* IF (((i - Pos2)=Options.ActSynt *) ;
            INC(j);
          END (* WHILE ((Options.ActSyntaxColour *) ;
        ELSIF MyText[i]=Options.ActSyntaxColouring.StringDelims[0] THEN
          (* KlS, 2003NOV03 *)
        ...
        ELSIF MyText[i]=Options.ActSyntaxColouring.StringDelims[1] THEN
          (* KlS, 2003NOV03 *)
        ...
        END (* IF Syntax.IsIdentChar(MyText[i] *) ;
        INC(i);
      END (* WHILE i<Length *) ;
      win.SelectColor(TextColor);
...
END (* CASE ColorELSEwin.SelectColor(C *) ;
...
BEGIN
  ...
  rect.right   := x2 + 1;
  rect.bottom  := y2 + 1;
  dummy        := WinUser.FillRect(win.hdc, rect, win.backBrush);
  (* Rechteck füllen *)
END BackRect;
...
BEGIN
  ...
  WHILE (CommentString[sInx]#0X) & (CommentString[sInx]=txt[sInx + inx]) DO
    INC(sInx);
  END (* WHILE (CommentString[sInx]#0X)  *);
...
BEGIN
    ...
    WHILE txt[i]#0X DO
      ...
      ELSE
        ...
        ELSE
          colFlag := 3
        END (* IF nesting>0 *) ;
      END (* IF win.text.isSelected & (((row *) ;
   ...
   & (row>=win.text.markStart.row) & (row<=win.text.markEnd.row) THEN
    IF win.text.markStart.row=win.text.markEnd.row THEN
      ...
      IF len1>0 THEN
        ...
        END (* IF Options.colorComments & (nes *);
      END (* IF len1>0 *) ;
      IF len2>0 THEN
  ...
  ELSE
    ...
    END (* IF Options.colorComments & (nes *) ;
  END (* IF Options.colorComments & isCo *) ;
...
BEGIN
  IF (row<win.textPos) OR (row>win.textPos + win.lineNo) THEN
    RETURN 
  END (* IF (row<win.textPos) OR (row>wi *) ;

  Done         := WinUser.HideCaret(win.hwnd);
  (* Caret vom Bildschirm verbergen *)
  IF ~win.text.GetLineEx(row, localTxt, len, isCommented, nesting, h) THEN
    localTxt   := ""
  END (* IF ~win.text.GetLineEx(row, loc *);
...
PROCEDURE (VAR win: WinDescT) ShowTextRange*
                                      (startRow,
                                       endRow:             LONGINT);
VAR
  i,
...
BEGIN
  ...
  ELSE
    ...
    win.wndwidth - 1, win.wndheight - 1);
  END (* IF win.textPos + win.lineNo>win *);
  IF (startRow>win.textPos + win.lineNo) OR (endRow<win.textPos) THEN
    ok         := WinUser.ShowCaret(win.hwnd);
    (* Caret anzeigen *)
    RETURN ;
  END (* IF (startRow>win.textPos + win. *);
  IF startRow<win.textPos THEN
    startRow   := win.textPos
  END (* IF startRow<win.textPos *);
  IF endRow>win.textPos + win.lineNo THEN
    endRow     := win.textPos + win.lineNo
  END (* IF endRow>win.textPos + win.lin *);
  IF win.text.GetLineEx(startRow, localTxt, len, isCommented, nesting, h) THEN
    ...
    WHILE (startRow<=endRow) & done DO
      ...
      END (* IF startRow<=endRow *);
    END (* WHILE (startRow<=endRow) & done *);
  ELSE
    win.BackRect(0, win.initrowpos + win.lineheight*(startRow - win.textPos), 
    win.wndwidth - 1, win.wndheight - 1);
  END (* IF win.text.GetLineEx(startRow, *);
  ok           := WinUser.ShowCaret(win.hwnd);
  (* Caret anzeigen *)
END ShowTextRange;
...
PROCEDURE (VAR win: WinDescT) VerScroll*
                                      (n:                  LONGINT);
BEGIN
...
PROCEDURE (VAR win: WinDescT) VerScrollThumb*
                                      (scrpos:             LONGINT);
BEGIN
  win.textPos  := scrpos;
...
PROCEDURE (VAR win: WinDescT) HorScroll*
                                      (n:                  LONGINT);
BEGIN
...
PROCEDURE (VAR win: WinDescT) HorScrollThumb*
                                      (scrpos:             LONGINT);
BEGIN
  win.colPos   := scrpos;
  win.UpdateHorScrollBar;
...
BEGIN
  ...
  IF win.text.markEnd.col<=1 THEN
    DEC(end)
  END (* IF win.text.markEnd.col<=1 *);
  FOR i:=win.text.markStart.row TO end DO
    dummy      := win.text.InsertInLine(t, 1, i);
    changed    := TRUE;
  END (* FOR i:=win.text.markStart.row T *);
  IF changed THEN
...
PROCEDURE (VAR win: WinDescT) MarkUpdate*
                                      (row,
                                       col:                LONGINT);
VAR
  swap:                                BOOLEAN;
...
BEGIN
  ...
  IF swap THEN
    win.markDown := ~win.markDown
  END (* IF swap *);
  IF oldStartRow>win.text.markStart.row THEN
    oldStartRow := win.text.markStart.row
  END (* IF oldStartRow>win.text.markSta *);
  IF oldEndRow<win.text.markEnd.row THEN
    oldEndRow  := win.text.markEnd.row
  END (* IF oldEndRow<win.text.markEnd.r *);
...
PROCEDURE (VAR win: WinDescT) InsertText*
                                      (source:             WinDef.LPSTR)
                                      :BOOLEAN;
VAR
  dmy:                                 WinDef.LPSTR;
...
(*---------------------------------------------------------------------------*)
PROCEDURE NextLineFromSource          (VAR line:           ARRAY OF CHAR)
                                      :BOOLEAN;
VAR
  pos,
  endpos:                              LONGINT;
...
BEGIN
  ...
  WHILE (text^[i]#0DX) & (text^[i]#0AX) & (text^[i]#0X) DO
    IF i - pos<ListSt.MAXLENGTH - 1 THEN
      line[i - pos] := text^[i];
      endpos   := i - pos;
    END (* IF i - pos<ListSt.MAXLENGTH - 1 *);
    INC(i);
  END (* WHILE (text^[i]#0DX) & (text^[i *);
  moreText     := text^[i]#0X;
  IF text^[i]=0DX THEN
    INC(i)
  END (* IF text^[i]=0DX *);
...
BEGIN
  ...
  IF NextLineFromSource(localTxt) THEN
    ...
    IF update THEN
      IF win.col + linelen>=ListSt.MAXLENGTH THEN
        ...
        RETURN FALSE
      END (* IF win.col + linelen>=ListSt.MA *);
      IF win.col - 1<=strlen THEN
        (* at/before 0X *)
        merge  := TRUE;
      ...
      ELSE
        ...
        FOR j:=strlen TO win.col-2 DO
          localStrLine[j] := " ";
        END (* FOR j:=strlen TO win.col-2 *);
      END (* IF win.col - 1<=strlen *);
      FOR j:=win.col-1 TO win.col-1+linelen DO
        localStrLine[j] := localTxt[j-win.col+1];
      END (* FOR j:=win.col-1 TO win.col-1+l *);
    ...
    WHILE NextLineFromSource(localTxt) DO
      (* insert whole lines *)
      IF offset=1 THEN
      ...
      END (* IF ~dmyb *);
      INC(offset);
    END (* WHILE NextLineFromSource(localT *);
  ...
  ELSE
    ...
    IF update THEN
      IF strlen + linelen>=ListSt.MAXLENGTH THEN
        GlobWin.DisplayError("ERROR", "Buffer exceeds maximal linelength");
        RETURN FALSE;
      END (* IF strlen + linelen>=ListSt.MAX *);
      IF win.col - 1<=strlen THEN
        (* bei/bevor 0X *)
        FOR j:=strlen TO win.col-1 BY-1 DO
          (* shift back *)
          localStrLine[j+linelen] := localStrLine[j];
        END (* FOR j:=strlen TO win.col-1 BY-1 *);
        xoffset := win.col - 1;
      ...
      ELSE
        ...
        IF win.col + linelen>ListSt.MAXLENGTH THEN
          GlobWin.DisplayError("ERROR", "Buffer exceeds maximal linelength");
          RETURN FALSE;
        END (* IF win.col + linelen>ListSt.MAX *);
        xoffset := win.col - 1;
        FOR j:=win.col-1 TO win.col-1+linelen DO
          localStrLine[j] := localTxt[j-win.col+1];
        END (* FOR j:=win.col-1 TO win.col-1+l *);
    ...
    ELSE
      ...
      ASSERT(dmyb);
    END (* IF update *);
  END (* IF NextLineFromSource(localTxt) *);
  ...
  IF win.textPos + win.lineNo<=win.row THEN
    ...
    (* Aktualisierungsbereich festlegen *)
    win.UpdateVerScrollBar;
  END (* IF win.textPos + win.lineNo<=wi *);
  win.ShowTextRange(win.textPos, win.textPos + win.lineNo);
  win.SetCaret;
  RETURN TRUE;
END InsertText;
...
PROCEDURE (VAR win: WinDescT) InsertGlobMem*
                                      (globMem:            WinDef.HGLOBAL)
                                      :BOOLEAN;
VAR
  dummy,
  lpCopy:                              LONGINT;
...
BEGIN
  lpCopy       := WinBase.GlobalLock(globMem);
