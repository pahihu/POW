(*===========================================================================
 ...
 ============================================================================
  ...
  2001JUN03 KLS     ERROR in Garbage Collector eliminated
 ===========================================================================*)
...
BEGIN
  ...
  SYSTEM.GETREG(5, ebp);
  SYSTEM.GETREG(4, esp);
  IF (ebp#0) THEN                      (* KLS, 2001JUN03 *)
    ScanMemory(esp, ebp);              (* maybe esp + 4  *)
    esp := ebp;
    REPEAT                             (* KLS, 2001JUN03 *)
                                       (* could be optimized: the stack of the procedure GC need not be scanned *)
      SYSTEM.GET(ebp, ebp);
      ScanMemory(esp, ebp);            (* maybe esp + 4  *)
      esp := ebp;
    UNTIL ebp=0;
  END (* IF (ebp#0) *);                (* KLS, 2001JUN03 *)
