...
(*---------------------------------------------------------------------------*)
(* 1997AUG20  LEI  rel. 32/1.0                                               *)
(* 1998NOV19  LEI  rel. 32/1.1 LEI bug in RemoveTrailingSpaces fixed         *)
(* 2001AUG01  KlS  Additional function UHexStr displaying an unsigned hex    *)
(*                 number                                                    *)
(**---------------------------------------------------------------------------  
...
PROCEDURE ReverseStringT              (VAR t:              StringT; 
                                       n:                  LONGINT);
VAR
  a,b:LONGINT;
  x:CHAR;
...
PROCEDURE UHexStr*                    (x:                  LONGINT;
                                       Size:               SHORTINT;
                                       VAR t:              StringT);
(** The number <x> is treated as a unsigned integer and converted to a string of 
    hexadecimal format and the result is stored  in <t>. At the end of the string 
    an "H" is appended to indicate the hexadecimal representation of the number.
    <Size> holds the number of bytes the variable "x" consists of.
...
BEGIN
  maxlen   := LEN(t)-1;
  IF ((maxlen<2) OR ((Size>4) OR (Size<1))) THEN
    IF maxlen=1 THEN t[0]:="$"; t[1]:=0X ELSE t[0]:=0X END;
    RETURN;
  END;
  IF (maxlen<(2*Size+1)) THEN
    FOR i:=0 TO maxlen-1 DO t[i]:="$" END;
    t[maxlen]:=0X;
    RETURN;
  END;
