...
(* 2001JAN29   KlS     kindOfNumberL corrected                                *)
(**---------------------------------------------------------------------------  
...
BEGIN
  ...
  CodeExamine;
  CodeStoreSW_V1;
  CodeSTP_P1LongReal;                                      (* KlS, 2001JAN29 LongReal must be stored instead of *)
                                                           (*                Real as in the original code       *)
  k:=0;
  IF SYSTEM.BIT(ss,8) THEN k:=1; END;
  IF SYSTEM.BIT(ss,10) THEN k:=k+2; END;
  IF SYSTEM.BIT(ss,14) THEN k:=k+4; END;
...
BEGIN
  ...
  xa:=ABS(x);
  IF (xa>=MIN(REAL)) & (xa<=MAX(REAL)) THEN RETURN ISREAL END;
  IF (xa>=MIN(LONGREAL)) & (xa<=MAX(LONGREAL)) THEN RETURN ISLONGREAL END;
  RETURN ISOUTOFRANGE;
END ValResult;
...
BEGIN
  ...
  IF exponent#0 THEN x:=x*ExpL(10,exponent) END;
  xa:=ABS(x);
  IF (xa>=MIN(LONGREAL)) & (xa<=MAX(LONGREAL)) THEN RETURN x END;
  RETURN MIN(LONGREAL);
END Val;
