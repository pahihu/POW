...
(* 2001JAN10   KlS Reading the commandline uses a WinBase Command            *)
(**--------------------------------------------------------------------------  
...
TYPE
                                                           (* KlS, 2001JAN10 *)
  TPcurrentCmd =                       POINTER TO ARRAY MAXPARAMLEN OF CHAR;

VAR
  currentCmd:                          ARRAY MAXPARAMLEN OF CHAR;
  PcurrentCmd:                         TPcurrentCmd;       (* KlS, 2001JAn10 *)
  PCmdLine:                            WinDef.LPSTR;       (* KlS, 2001JAN10 *)
...
BEGIN
  ...
  WHILE (line[i]#0X) & (strN<MAXSTR) DO
    ...
    Strings.Copy(line,strList[strN]^,start+1,end-start+1);
...
PROCEDURE Str*                        (paramNr:            INTEGER; 
                                       VAR paramTxt:       ARRAY OF CHAR);
...
BEGIN
  IF (paramNr<1) OR (paramNr>strN) THEN
    paramTxt[0]:=0X;
  ELSE
    IF strList[paramNr-1]#NIL THEN 
      COPY(strList[paramNr-1]^, paramTxt) 
    ELSE 
      paramTxt[0]  := 0X
    END;
  END;
...
BEGIN;
  strN         := 0;
  PCmdLine     := WinBase.GetCommandLineA();               (* KlS, 2001JAN10 *)                        
  PcurrentCmd  :=SYSTEM.VAL(TPcurrentCmd, PCmdLine);
  Parse(PcurrentCmd^);                                     (* KlS, 2001JAN10 *)
END Param.
