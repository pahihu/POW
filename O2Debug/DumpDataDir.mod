...
VAR
  ...
  PMyCharArray:                        Common.PCharArray;
  PMyImportHintName:                   PImportHintName;
  PMyStringArray:                      Common.PStringArray;
  PMyDWordArray:                       Common.PDWordArray;
  PMyWord:                             Common.PWord;
...
PROCEDURE WriteLines00*               (FirstLineP:         Common.ScreenLineP;
...
VAR
  ...
  POrdinals:                           Common.PWordArray;
  PFunctions:                          Common.PDWordArray;
  PNames:                              Common.PDWordArray;
...
BEGIN;
  ...
  NumberOfLines                :=  0;
  Done                         := FALSE;
  MyPointer                    := Dump.ImageDirectoryOffset(0, PMyImageSectionHeader);
  ...
  MyPointer2   := PMyImage_Export_Directory^.Name - Delta + Common.MyFileDescription.lpFileBase;
  PMyCharArray := SYSTEM.VAL(Common.PCharArray, MyPointer2);
  Common.AppendPtrString(ActLine^.Text, PMyCharArray^);
  ...
  MyPointer2   := PMyImage_Export_Directory^.AddressOfNameOrdinals - Delta + Common.MyFileDescription.lpFileBase;
  POrdinals    := SYSTEM.VAL(Common.PWordArray, MyPointer2);
  MyPointer2   := PMyImage_Export_Directory^.AddressOfFunctions - Delta + Common.MyFileDescription.lpFileBase;
  PFunctions   := SYSTEM.VAL(Common.PDWordArray, MyPointer2);
  MyPointer2   := PMyImage_Export_Directory^.AddressOfNames - Delta + Common.MyFileDescription.lpFileBase;
  PNames       := SYSTEM.VAL(Common.PDWordArray, MyPointer2);
  ...
  FOR i:=0 TO PMyImage_Export_Directory^.NumberOfNames-1 DO
    ...
    MyPointer2   := PNames[i] - Delta + Common.MyFileDescription.lpFileBase;
    PMyCharArray := SYSTEM.VAL(Common.PCharArray, MyPointer2);
    Common.AppendPtrString(ActLine^.Text, PMyCharArray^);
    Dump.NextLine(ActLine, NumberOfCol, NumberOfLines);
  END;
...
PROCEDURE WriteLines01*               (FirstLineP:         Common.ScreenLineP;
                                       VAR NumberOfCol:    LONGINT)
                                      :                    LONGINT;
...
BEGIN;
  ...
  NumberOfLines                :=  0;
  Done                         := FALSE;
  MyPointer                    := Dump.ImageDirectoryOffset(1, PMyImageSectionHeader);
  ...
  REPEAT
    ...
    MyPointer2   := PMyImage_Import_Descriptor^.Name - Delta + Common.MyFileDescription.lpFileBase;
    PMyCharArray := SYSTEM.VAL(Common.PCharArray, MyPointer2);
    Common.AppendPtrString(ActLine^.Text, PMyCharArray^);
    ActLine^.Format  := Common.Header2;
    Dump.NextLine(ActLine, NumberOfCol, NumberOfLines);
    ...
    ActLine^.Format  := Common.Header3;
    Dump.NextLine(ActLine, NumberOfCol, NumberOfLines);
    MyPointer2       := SYSTEM.VAL(LONGINT, PMyImage_Import_Descriptor^.FirstThunk) - Delta + Common.MyFileDescription.lpFileBase;
    PMyDWordArray    := SYSTEM.VAL(Common.PDWordArray, MyPointer2);
    j                :=  0;
    ActLine^.Format  := Common.Text01;
    REPEAT
      IF SYSTEM.BIT(PMyDWordArray[j], 31) THEN
        ...
        Strings.Append(ActLine^.Text, MyChars);
      ELSE
        MyPointer2         := SYSTEM.VAL(LONGINT, PMyDWordArray[j]) - Delta + Common.MyFileDescription.lpFileBase;
...
PROCEDURE WriteLines02*               (FirstLineP:         Common.ScreenLineP;
                                       VAR NumberOfCol:    LONGINT)
                                      :                    LONGINT;
...
BEGIN;
  ...
  Done                         := FALSE;
  Level                        :=  1;
  MyPointer                    := Dump.ImageDirectoryOffset(2, PMyImageSectionHeader);
...
PROCEDURE WriteLines05*               (FirstLineP:         Common.ScreenLineP;
                                       VAR NumberOfCol:    LONGINT)
                                      :                    LONGINT;
