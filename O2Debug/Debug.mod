...
IMPORT
  Common, DumpDataDirDebug, Resource, UIStatusLine, View,
  Strings,
...
CONST
  Version*     =                      "V 2.00.16";
  Module*      =                      "Debug";
  ErrorNoOffset=                      Resource.IDM_Debug * 100;
  
  TimeOut      =                       1000;               (* Timeout in Millisec *)
...
VAR
  szEvent:                             ARRAY Common.MaxPathLength OF CHAR;
...
VAR
  MyDebugThread,
  ActDebugThread:                      Common.DebugThreadP;
  ...
  DebugProcessP:                       Common.DebugProcessP;
...
BEGIN
  ...
  IF hEvent=0 THEN
    UIStatusLine.DisplayError (1, 3010);
  END;
  ...
  IF WinBase.CreateProcessA  (WinDef.NULL,
    ...
    IF ErrorCode#0 THEN
      UIStatusLine.DisplayError(ErrorCode, 3020);
    END (* IF ErrorCode#0 *);
  ...
  Result       := WinBase.SetEvent (hEvent);
  IF Result=0 THEN
    UIStatusLine.DisplayError (1, 3030);
  END;
  Result        := WinBase.CloseHandle (hEvent);
  ...
  LOOP
    ...
    CASE nIndex OF
      ...
      CLOSEDEBUGGER:
        ...
        hEvent := WinBase.OpenEventA (WinNT.EVENT_MODIFY_STATE, WinDef.False, SYSTEM.ADR(szEvent));
        IF hEvent=0 THEN
          UIStatusLine.DisplayError (1, 3040);
        END;
        Result := WinBase.SetEvent (hEvent);
        ...
        Result := WinBase.SetEvent (hEvent);
        IF Result=0 THEN
          UIStatusLine.DisplayError (1, 3050);
        END;
            ...
            CASE DebugEvent.dwDebugEventCode OF
              ...
              WinBase.EXCEPTION_DEBUG_EVENT:
                IF DebugProcessP^.FirstBreakpoint THEN
                  ...
                  Strings.Append(LineOfText, MyChars);
                  View.AppendLine (DebugProcessP^.hWnd, LineOfText);
                  Done       := DumpDataDirDebug.GetLineNumber(ExceptionRecordP^.ExceptionAddress, LineOfText, LineNumber);
...
(*                                                                           *)
(*****************************************************************************)
PROCEDURE Start*                      (VAR MyDebugProcess: Common.DebugProcessP)
                                      :WinDef.HANDLE;
...
BEGIN
  ...
  COPY (Common.MyFileDescription.Path, MyDebugProcess^.szModule);
  Strings.Append(MyDebugProcess^.szModule, Common.MyFileDescription.Name);
  ...
  IF MyDebugProcess^.hThread=0 THEN
    UIStatusLine.ShowMessage("Debug.StartDebugger: Starting Thread failed.");
    RETURN 0;
  END (* IF MyDebugProcess^.hThread=0 *);
  
  IF MyDebugProcess^.dwProcessID=0 THEN
    UIStatusLine.ShowMessage("Debug.StartDebugger: Process creation failed.");
    RETURN 0
  END (* IF MyDebugProcess^.dwProcessID=0 *);

  UIStatusLine.ShowMessage("Debug.StartDebugger: Thread started.");
  (* Wait for debugger to complete initialization *)
  Result := WinBase.WaitForSingleObject (hEvent, TimeOut);
  Result := WinBase.CloseHandle(hEvent);
  IF MyDebugProcess^.dwProcessID=0 THEN
    UIStatusLine.ShowMessage("Debug.StartDebugger: Process creation failed.");
    RETURN 0
  ...
  UIStatusLine.ShowMessage("Debug.StartDebugger: Thread started & set event handle .");
...
(*                                                                           *)
(*****************************************************************************)
PROCEDURE Resume*                     (MyDebugProcess:     Common.DebugProcessP)
                                      :WinDef.LRESULT;
...
(*                                                                           *)
(*****************************************************************************)
PROCEDURE Stop*                       (VAR MyDebugProcess: Common.DebugProcessP)
                                      :WinDef.HANDLE;
...
BEGIN
  ...
  IF Common.DebugProcess^.dwProcessID=0 THEN
    RETURN 0;
  END (* IF Common.DebugProcess^.dwProcessID=0  *);
  ...
  UIStatusLine.ShowMessage("Debug.StopDebugger: Event IDS_DBGEVNTCLOSEACK generated.");

  (* Set close event for debug thread and wait ... *)
  Result       := WinBase.SetEvent (DebugEvents[CLOSEDEBUGGER]);
  Result       := WinBase.WaitForSingleObject (hEvent, TimeOut);
  UIStatusLine.ShowMessage("Debug.StopDebugger: Thread closed.");
