...
(*                                                                           *)
(* MODULE:     DumpDataDirDebug                            V 2.00.07         *)
(*                                                         2003APR18         *)
...
TYPE
  ...
  (* length module name.                                                     *)
  POMFSegDescArray     =               POINTER TO ARRAY Common.maxArrayLength OF OMFSegDesc;
  ...
  POMFSourceLineArray     =            POINTER TO ARRAY Common.maxArrayLength OF POMFSourceLine;
  ...
  POMFSourceFileArray     =               POINTER TO ARRAY Common.maxArrayLength OF POMFSourceFile;
...
VAR
  ...
  OMFSourceFileName:                   Common.PCharArray;
  OMFSourceLineOffset:                 Common.PDWordArray;
  OMFSourceLineLineNumber:             Common.PWordArray;
  ActDATASYM32:                        PDATASYM32;
  ActUDTSYM:                           PUDTSYM;
  ActREFSYM:                           PREFSYM;
  OMFMpcDebugInfoWORD:                 Common.PWordArray;
  OMFLibraryCHAR:                      Common.PCharArray;
  OMFModuleCHAR:                       Common.PCharArray;
  SymbolString:                        Common.PCharArray;
...
*)
    CASE ActOMFDirEntry^.subsection OF
      ...
      sstSrcModule:
        ...
        OMFSourceFileName   := SYSTEM.VAL(Common.PCharArray, SYSTEM.ADR(ActOMFSourceFile.Name));
        Strings.Append(ActLine^.Text, "   ");
...
PROCEDURE WriteLines06*               (FirstLineP:         Common.ScreenLineP;
                                       VAR NumberOfCol:    LONGINT)
                                      :                    LONGINT;
...
VAR
  ...
  MyPDWordArray:                       Common.PDWordArray;
...
BEGIN;
  ...
  IF Common.MyFileDescription.FileType=Common.FileTypeOBJ THEN
    PDebugDirectory  := PDebugDirectory^.Next;
    PDebugDirectory  := PDebugDirectory^.Next;
    MyPDWordArray    := SYSTEM.VAL(Common.PDWordArray, PDebugDirectory^.PDebugDirectory);
    FOR i:=0 TO 20 DO
  ...
  REPEAT
    ...
    CASE PDebugDirectory^.PDebugDirectory^.Type OF
      WinNT.IMAGE_DEBUG_TYPE_CODEVIEW:                     (* 02 *)
        lfaBase    := Common.MyFileDescription.lpFileBase + PDebugDirectory^.PDebugDirectory^.PointerToRawData;
        MyPointer := SYSTEM.VAL(LONGINT, lfaBase);
        UIStatusLine.ShowMessage1Hex ("Base: #", MyPointer);
...
PROCEDURE WriteLinesLN*               (FirstLineP:         Common.ScreenLineP;
                                       VAR NumberOfCol:    LONGINT)
                                      :                    LONGINT;
...
BEGIN
  ...
  FOR i:=1 TO MyOMFDirHeader^.cDir DO
    ...
    IF ActOMFDirEntry^.subsection=sstSrcModule THEN        (* describes the source line number to address mapping *)
      ...
      OMFSourceLineOffset     := SYSTEM.VAL(Common.PDWordArray, SYSTEM.ADR(ActOMFSourceLine.offset));
      OMFSourceLineLineNumber := SYSTEM.VAL(Common.PWordArray, SYSTEM.ADR(OMFSourceLineOffset[ActOMFSourceLine^.cPair]));
      ...
      WHILE k<ActOMFSourceLine^.Seg DO
        ...
        INC(k);
      END;
      Offset           := Common.PMyNT_Header.OptionalHeader.ImageBase + ActSectionHeader^.PSectionHeader.VirtualAddress;
...
PROCEDURE WriteLinesST*               (FirstLineP:         Common.ScreenLineP;
                                       VAR NumberOfCol:    LONGINT)
                                      :                    LONGINT;
...
BEGIN
  ...
  FOR i:=1 TO MyOMFDirHeader^.cDir DO
    ...
    IF ActOMFDirEntry^.subsection=sstGlobalPub THEN        (* describes the public symbol to address mapping *)
      ...
      Offset           := Common.PMyNT_Header.OptionalHeader.ImageBase;
      ...
      END (* While j<ActOMFGlobalPub^.cbSymbol-20 *);
    END;
    INC(MyPointer, SIZE(OMFSegDesc));
...
PROCEDURE WriteLinesST2*              (FirstLineP:         Common.ScreenLineP;
                                       VAR NumberOfCol:    LONGINT)
                                      :                    LONGINT;
...
BEGIN
  ...
  IF Common.PMyNT_Header^.FileHeader.NumberOfSymbols=0 THEN
    COPY (LineEmpty, ActLine^.Text);
    Strings.Append(ActLine^.Text, "  not found.");
    Dump.NextLine(ActLine, NumberOfCol, NumberOfLines);
    COPY (LineEmpty, ActLine^.Text);
    ...
    RETURN NumberOfLines
  ELSE
    Strings.UHexStr(Common.PMyNT_Header^.FileHeader.NumberOfSymbols, 4, MyChars);
    Strings.Append(ActLine^.Text, MyChars);
